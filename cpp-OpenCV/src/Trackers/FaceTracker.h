...
public:
    ...
    // Метод для настройки параметров оптимизации
    void setOptimizationParams(int skipFrames = 3, bool optimizedDetection = true);
...
private:
    ...
    int detectionSkipFrames;         // Сколько кадров пропускать между детектированиями
    int framesSinceLastDetection;    // Счетчик кадров с последнего детектирования
    bool enableOptimizedDetection;   // Использовать оптимизированные параметры детектирования

    // Дополнительные параметры трекинга
    float confidenceThreshold;
    int minTrackFramesForStable;
    ...
    void updateTracksWithDetection(const std::vector<cv::Rect>& detectedFaces, float deltaTime);
    void predictTracksWithoutDetection(float deltaTime);
